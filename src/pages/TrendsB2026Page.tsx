    ...
    <div style={{ background: BG_DARK, minHeight: "100vh", fontFamily: FF, color: "#fff", display: "flex", flexDirection: "column", overflowX: "hidden" }}>
      ...
      {isMobile ? (
        ...
        <main style={{ flex: 1, paddingBottom: 120 }}>
          ...
          {/* БЛОК БОЛИ */}
          <div style={{ background: "#0e0e0e", padding: "28px 20px 32px", position: "relative", overflow: "hidden" }}>
            <div style={{
            ...
            <div style={{ position: "relative", zIndex: 1 }}>
              ...
              <div>
                ...
                <span className="pain-line" style={{ color: "rgba(255,255,255,0.5)" }}>А доход не растёт третий год</span>
              </div>
          ...
          {/* БЛОК СПИКЕРА */}
          <div style={{ background: "#131313", borderTop: "1px solid rgba(255,255,255,0.06)", padding: "28px 20px 32px" }}>
            <div style={{ position: "relative", height: 300, borderRadius: 12, overflow: "hidden", marginBottom: 20 }}>
              ...
              <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 80, background: "linear-gradient(to top, #131313, transparent)" }} />
            </div>
            <div style={{ fontSize: 18, fontWeight: 800, color: "#fff", marginBottom: 4 }}>Анна Симонова</div>
            <div style={{ fontSize: 12, color: ACCENT, fontWeight: 600, letterSpacing: "0.04em", textTransform: "uppercase", marginBottom: 16 }}>Основатель RAD ACADEMY</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {SPEAKER_CREDS.map((c, i) => (
                <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                  <span style={{ color: ACCENT, fontSize: 14, marginTop: 2, flexShrink: 0 }}>—</span>
                  <span style={{ fontSize: 13, color: "rgba(255,255,255,0.65)", lineHeight: 1.5 }}>{c}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ padding: "28px 20px 0", textAlign: "center" }}>
            <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.04em", textTransform: "uppercase", color: ACCENT, marginBottom: 8 }}>Бесплатный эфир</div>
            <div style={{ fontSize: 20, fontWeight: 800, lineHeight: 1.2, textTransform: "uppercase", marginBottom: 12 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 20 }}>
              {["📍 Сочи: 480 000 ₽ первый проект", "📍 Казань: чек ×4,7", "📍 Екатеринбург: доход ×3,8"].map((t, i) => (
                <div key={i} style={{ fontSize: 13, color: "rgba(255,255,255,0.6)" }}>{t}</div>
              ))}
            </div>
            <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: "14px 16px", marginBottom: 20, textAlign: "left" }}>
        ...
        <main style={{ flex: 1, display: "flex", flexDirection: "column" }}>
          ...
          {/* ── РЯД 1: ДВУХЗОННЫЙ БЛОК ── */}
          <div style={{ display: "flex", minHeight: "52vh" }}>

            {/* ЛЕВАЯ ЗОНА — БОЛЬ */}
            <div style={{ width: "42%", flexShrink: 0, background: "#0e0e0e", padding: "48px 48px 48px 56px", display: "flex", flexDirection: "column", justifyContent: "center", position: "relative", overflow: "hidden" }}>
              <div style={{ position: "absolute", inset: 0, opacity: 0.035, backgroundImage: "repeating-linear-gradient(0deg, #fff 0px, #fff 1px, transparent 1px, transparent 40px), repeating-linear-gradient(90deg, #fff 0px, #fff 1px, transparent 1px, transparent 40px)" }} />
              {/* диагональный переход */}
              <div style={{ position: "absolute", top: 0, right: -1, bottom: 0, width: "30%", background: `linear-gradient(to right, transparent 0%, ${BG_RIGHT} 100%)`, zIndex: 2 }} />
              <div style={{ position: "relative", zIndex: 1 }}>
                <div style={{ fontSize: 11, letterSpacing: "0.2em", color: "#c0392b", fontWeight: 700, textTransform: "uppercase", marginBottom: 28 }}>ЗНАКОМО?</div>
                <div style={{ marginBottom: 28 }}>
                  <span className="pain-line">Квартира 80 м²</span>
                  <span className="pain-line">Чек 150–200 тыс. ₽</span>
                  <span className="pain-line">Очередь на 4 месяца</span>
                  <span className="pain-line" style={{ color: "rgba(255,255,255,0.42)" }}>А доход не растёт третий год</span>
                </div>
                <div style={{ fontSize: 13, color: "#3a3a3a", fontStyle: "italic" }}>Так живут 80% дизайнеров жилых интерьеров</div>
              </div>
            </div>

            {/* ПРАВАЯ ЗОНА — РЕШЕНИЕ */}
            <div style={{ flex: 1, background: BG_RIGHT, padding: "48px 56px 48px 64px", display: "flex", alignItems: "center", position: "relative", overflow: "hidden" }}>
              <div style={{ position: "absolute", top: -100, right: -100, width: 500, height: 500, background: "radial-gradient(circle, rgba(232,86,58,0.1) 0%, transparent 65%)", pointerEvents: "none" }} />
              <div style={{ position: "absolute", bottom: -80, left: 60, width: 300, height: 300, background: "radial-gradient(circle, rgba(232,86,58,0.06) 0%, transparent 65%)", pointerEvents: "none" }} />
              <div style={{ position: "relative", zIndex: 1 }}>
                <div style={{ fontSize: 11, letterSpacing: "0.2em", color: ACCENT, fontWeight: 700, textTransform: "uppercase", marginBottom: 28 }}>А МОЖНО ТАК:</div>
                <div>
                  ...
                  <div style={{ fontSize: "clamp(17px, 1.5vw, 22px)", fontWeight: 700, color: "#fff", marginTop: 24 }}>Те же навыки. Другой рынок.</div>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* ── РЯД 2: БЛОК СПИКЕРА ── */}
          <div style={{ display: "flex", background: "#131313", borderTop: "1px solid rgba(255,255,255,0.06)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>

            {/* Фото */}
            <div style={{ width: "35%", flexShrink: 0, position: "relative", overflow: "hidden", minHeight: 340 }}>
              <img
                src={SPEAKER_PHOTO}
                alt="Анна Симонова"
                style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center top", display: "block", position: "absolute", inset: 0 }}
              />
              <div style={{ position: "absolute", top: 0, right: 0, bottom: 0, width: "25%", background: "linear-gradient(to right, transparent, #131313)" }} />
            </div>

            {/* Регалии */}
            <div style={{ flex: 1, padding: "44px 56px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
              <div style={{ fontSize: "clamp(20px, 1.8vw, 28px)", fontWeight: 800, color: "#fff", marginBottom: 6 }}>Анна Симонова</div>
              <div style={{ fontSize: 12, color: ACCENT, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 28 }}>
                Основатель RAD ACADEMY · Практикующий дизайнер интерьеров
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {SPEAKER_CREDS.map((c, i) => (
                  <div key={i} style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
                    <span style={{ color: ACCENT, fontSize: 16, marginTop: 1, flexShrink: 0, fontWeight: 700 }}>—</span>
                    <span style={{ fontSize: "clamp(13px, 1.1vw, 15px)", color: "rgba(255,255,255,0.65)", lineHeight: 1.55 }}>{c}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* ── РЯД 3: НИЖНЯЯ ПОЛОСА CTA ── */}
          <div style={{ background: "#0f0f0f", padding: "36px 40px", textAlign: "center", flexShrink: 0 }}>
            ...
            <button className="cta-pulse" onClick={handleCTA} style={{ background: ACCENT, color: "#fff", border: "none", borderRadius: 10, padding: "18px 56px", fontSize: 15, fontWeight: 800, letterSpacing: "0.07em", textTransform: "uppercase", cursor: "pointer", marginBottom: 16, fontFamily: FF }}>
              Участвовать в эфире бесплатно
            </button>
          ...
          <div style={{ background: "#1e1e1e", borderRadius: 16, padding: isMobile ? "32px 24px" : "40px 48px", maxWidth: 440, width: "100%", textAlign: "center", position: "relative", border: "1px solid rgba(255,255,255,0.08)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <button
                onClick={handleTg}
                disabled={!canSubmit}
                style={{ background: canSubmit ? "#229ED9" : "#2a2a2a", color: canSubmit ? "#fff" : "#555", border: "none", borderRadius: 10, padding: "15px 20px", fontSize: 15, fontWeight: 700, cursor: canSubmit ? "pointer" : "not-allowed", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, fontFamily: FF, transition: "background 0.2s, color 0.2s" }}
              >
              ...
              <button
                onClick={handleVk}
                disabled={!canSubmit}
                style={{ background: canSubmit ? "#0077FF" : "#2a2a2a", color: canSubmit ? "#fff" : "#555", border: "none", borderRadius: 10, padding: "15px 20px", fontSize: 15, fontWeight: 700, cursor: canSubmit ? "pointer" : "not-allowed", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, fontFamily: FF, transition: "background 0.2s, color 0.2s" }}
              >
